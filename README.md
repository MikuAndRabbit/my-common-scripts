...
5. **find-markdown-images**: 检测 Markdown 文件中的所有图片引用，按类型（本地/远程/内嵌）分类输出。支持行内图片、尖括号 URL、引用式图片、HTML `<img>` 标签等多种 Markdown 图片语法，可通过 `--json` 输出机器可读结果。底层为 Python 实现（位于 `python/find-markdown-images/`），由 `uv` 托管依赖。
